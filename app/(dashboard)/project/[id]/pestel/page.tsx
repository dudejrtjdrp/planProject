...
import { PestelAnalysisReport } from "@/components/frameworks/pestel/pestel-analysis-report";
...
import { getPestelItems, getPestelItemsByFrameworkId } from "@/features/pestel/data/pestel-repository";
...
export default async function ProjectPestelPage({ params, searchParams }: ProjectPestelPageProps) {
  ...
  const [pestelItems, profiles] = await Promise.all([
    pestelFramework ? getPestelItemsByFrameworkId(pestelFramework.id) : getPestelItems(id),
    getProfiles(),
  ]);
