...
import { fromSwotDbQuadrant, parseSwotContent, toSwotDbQuadrant } from "@/features/swot/types/swot-item";
...
export async function getSwotItems(projectId: string): Promise<SwotItem[]> {
	...
	const { data, error } = await supabase
		.from("swot_items")
		.select("id, project_framework_id, created_by, quadrant, title, description, content, position, created_at, updated_at")
...
export async function createSwotItem(
	...
	const parsed = parseSwotContent(content);
...
export async function cloneSwotItemsToFramework(
	...
	const { data, error } = await supabase
		.from("swot_items")
		.select("created_by, quadrant, title, description, content, position")
